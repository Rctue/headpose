...
def tansig_old(number):
    x = list()
    for i in range (0,len(number)):
        x.append((exp(number[i]) - exp(-number[i])) /(exp(number[i]) + exp(-number[i])))
    return numpy.asarray(x)
 
def mapminmax_old(array):
    #fixes division by int error: array = array.astype(float)
    difference = array.max() - array.min()
    array = array - (difference/2) # this is wrong, should be mean not difference
...
def tansig(array):
    return numpy.divide(2,(1+numpy.exp(numpy.multiply(-2,array))))-1
...
##        image = cv.LoadImage(filename)
    ...
    while True:
        ...
        image, center, isFlipped, isProfile, detected, region = Detect(image, True)
